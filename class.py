class Data:
    ...
    def channel_create(self, channel_name, is_public, creator_id):
        new_channel = Channel(channel_name, is_public, creator_id)
    ...
    def checkemailnotused(self, email):
        for user in self.users:
    ...
    def checkhandlenotused(self, handle):
        for user in self.users:
            if user.get_handle_str == handle:
                return 0
        return 1
...
class User:
    def __init__(self, name_first, name_last, email, password, token):
        ...
        self.reset_code = ''
        self.img_url = 'https://bpic.588ku.com//element_origin_min_pic/19/01/04/46eb55e4401f9f4ddf449d1ba54b9871.jpg'
